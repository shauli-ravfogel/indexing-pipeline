...
class BucketBatchSampler(Sampler):
    ...
    def _argsort_by_padding(
        self, instances: Iterable[Dict[(str, torch.tensor)]]
            ...
            for field_name in self.sorting_keys:
                if not hasattr(instance, field_name):
                    raise Exception(
                        ...
                        Available fields/keys are {list(instance.fields.keys())}.'
                    )
...
class MaxTokensBatchSampler(BucketBatchSampler):
    def __init__(self, data_source, max_tokens=None, sorting_keys=["input_ids"], padding_noise=0.1):
        ...
        for k, v in first.items():
            if k not in ("label", "label_ids", "guid") and v is not None:
                if isinstance(v, str) or isinstance(v, torch.Tensor):
                    ...
                    paddings_lengths = [max_length - length for length in lengths]
                    batch[k] = torch.tensor(
                        [f[k] + [PAD_ID] * pl for f, pl in zip(features, paddings_lengths)], dtype=(torch.long)
                    )
